...
function Posti(id_volo_andata='', id_div_posti='', volo_ritorno='', posti=''){
    ...
    if(id_div_posti_glob == 'andata' && volo_ritorno!=''){                       // Fa apparire i posti del volo d'andata se l'utente sceglie 'andata e ritorno'
    ...
    else if(id_div_posti_glob == 'ritorno' && volo_ritorno!=''){             // Fa apparire i posti del volo di ritorno se l'utente sceglie 'andata e ritorno'
    ...
    fetch(API_PREN)
        ...
        .then(data => {
            ...
            Genera_posti(id_volo_andata, posti_prenotati, id_div_posti_glob)
        })
...
function Genera_posti(id_volo_andata, posti_prenotati, id_div_posti_glob) {
    document.getElementById('prezzo_totale_'+id_div_posti_glob).value = 0;
    document.getElementById('posti_'+id_div_posti_glob).innerHTML = '';
    ...
    fetch(API_VOLI+id_volo_andata)
        ...
        .then(data => {
            ...
            document.getElementById(id_div_posti_glob).classList.remove('d-none');
            document.getElementById('tabella_posti').classList.remove('d-none');

            for (let i = 1; i <= content.posti; i++){       // Genera i posti
                document.getElementById('posti_'+id_div_posti_glob).append(Fila(i,content, posti_prenotati));
            }
        })
        .catch(err => console.log(err));
}
    ...
    for (let j = 0; j < 6; j++) {
        ...
        if(!posti_prenotati.includes(lettere[j] + i.toString())){       // Controlla se il posto è gia stato prenotato
            ...
            if (content.posti_prima_classe!=0){
                td += '<td><button type="button" id="'+ lettere[j] + i.toString() + '" value="'+ lettere[j] + i.toString() + '" class="btn btn-warning opacity-75" onclick="Scelta(this.id,'+content.prezzo_prima_classe+')"><img src="/static/img/poltrona.png" height=30 width=30></button></td>';
                content.posti_prima_classe--;
            }
            else if(content.posti_seconda_classe!=0){
                td += '<td><button type="button" id="'+ lettere[j] + i.toString() + '" value="'+ lettere[j] + i.toString() + '" class="btn btn-primary opacity-75" onclick="Scelta(this.id,'+content.prezzo_seconda_classe+')"><img src="/static/img/poltrona.png" height=30 width=30></button></td>';
                content.posti_seconda_classe--;
            }
            else{
                td += '<td><button type="button" id="'+ lettere[j] + i.toString() + '" value="'+ lettere[j] + i.toString() + '" class="btn btn-light opacity-75" onclick="Scelta(this.id,'+content.prezzo_terza_classe+')"><img src="/static/img/poltrona.png" height=30 width=30></button></td>';
            }
        }
        else{   
...
function Scelta(id, prezzo) {
    if (!posti_scelti.includes(id)) {  // Se l'id del posti è presente nell'array posti_scelti
        posti_scelti.push(id);          // Inserisce il posto scelto nell'array di posti_scelt
        document.getElementById('posti_prenotati_'+id_div_posti_glob).value = posti_scelti;      // Scrive il valore dell'array nell'input nascosto
        document.getElementById('div-posti-scelti').innerHTML = posti_scelti;
        let input_prezzo = document.getElementById('prezzo_totale_'+id_div_posti_glob).valueAsNumber;
        document.getElementById('prezzo_totale_'+id_div_posti_glob).value = input_prezzo + prezzo;    // Aggiorna il prezzo totale nell'input nascosto
        //document.getElementById('posti_'+id_div_posti_glob.id).querySelector('#'+id).classList.add('btn-lg');
        document.getElementById('posti_'+id_div_posti_glob).querySelector('#'+id).classList.remove('opacity-75');
        document.getElementById('posti_'+id_div_posti_glob).querySelector('#'+id).classList.add('border');
        document.getElementById('posti_'+id_div_posti_glob).querySelector('#'+id).classList.add('border-success');
        document.getElementById('posti_'+id_div_posti_glob).querySelector('#'+id).classList.add('border-5');
    }
    else {
        ...
        let input_prezzo = document.getElementById('prezzo_totale_'+id_div_posti_glob).valueAsNumber;
        document.getElementById('prezzo_totale_'+id_div_posti_glob).value = input_prezzo - prezzo;    // Aggiorna il prezzo totale nell'input nascosto
        //document.getElementById('posti_'+id_div_posti_glob.id).querySelector('#'+id).classList.remove('btn-lg');
        document.getElementById('posti_'+id_div_posti_glob).querySelector('#'+id).classList.remove('opacity-75');
        document.getElementById('posti_'+id_div_posti_glob).querySelector('#'+id).classList.remove('border');
        document.getElementById('posti_'+id_div_posti_glob).querySelector('#'+id).classList.remove('border-success');
        document.getElementById('posti_'+id_div_posti_glob).querySelector('#'+id).classList.remove('border-5');
    }
    ...
    if(posti_scelti.length == posti_global){    // Se il numero di posti selezionati è UGUALE il numero di posti scelti all'inizio 
        ...
        document.getElementById('posti_'+id_div_posti_glob).querySelector('#'+id).click();
    }
